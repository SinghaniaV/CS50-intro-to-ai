    ...
    if len(corpus[page]) < 1:
        ...
        for key in corpus.keys():
            if key not in corpus[page]:
                ...
                prob_dist[key] = even_factor + random_factor
...
def sample_pagerank(corpus, damping_factor, n):
    ...
    # turn sample count to probabilities
    ...
    while True:
        for page in corpus:
            ...
            for linking_page in corpus:
                # check if linking_page links to page
                if page in corpus[linking_page]:
                    temp += (old_dict[linking_page] / len(corpus[linking_page]))
                # if linking_page has no links to other pages at all, interpret it as having one link for every page
